...
export default class StandaloneSingleSpaPlugin {
    ...
    constructor(options) {
        ...
        if (typeof options.appOrParcelName !== "string") {
            throw Error(`${pluginName}: the options.appOrParcelName string must be provided`);
        }
        this.options = {
            ...defaultOptions,
            ...options,
    ...
    modifyScripts({ scripts }) {
        if (scripts.modifiedBySingleSpaStandalonePlugin) {
            throw Error(`standalone-single-spa-webpack-plugin: You have two separate instances of standalone-single-spa-webpack-plugin in your webpack config. If using webpack-config-single-spa or vue-cli-plugin-single-spa, you do not need to manually add the standalone plugin since it's already added by those projects.`);
        }
        if (scripts.length === 0) {
            throw Error(`standalone-single-spa-webpack-plugin: HtmlWebpackPlugin must generate at least one <script> in its output, but did not. This usually indicates that there is a problem with your webpack configuration.`);
        }
        const mainScript = scripts[0];
        if (mainScript.tagName !== "script" || !mainScript.attributes.src) {
            throw Error(`standalone-single-spa-webpack-plugin: HtmlWebpackPlugin contains an HTML element that's not a <script> with a 'src' attribute in its scripts array. Found ${mainScript.tagName}. This usually indicates a problem with your webpack configuration`);
        }
        ...
        const importMapPrefix = this.options.moduleFormat === "esm" ? "injector" : "systemjs";
        ...
        if (this.options.importMapUrls) {
            if (!Array.isArray(this.options.importMapUrls)) {
                throw Error(`standalone-single-spa-webpack-plugin: importMapUrls option must be an array, if present`);
            }
    ...
    applicationRuntime() {
      ...
      ${this.importMethod}('single-spa').then(function (singleSpa) {
        singleSpa.registerApplication({
          ...
          customProps: ${this.options.customProps
            ? JSON.stringify(this.options.customProps)
            : "{}"}
        });
        if (!window.location.pathname.startsWith('${this.options.activeWhen[0]}')) {
